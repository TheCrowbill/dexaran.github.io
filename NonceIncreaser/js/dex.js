...
*/
 ...
 $("#nameUpdateTxStatus").html('<p class="text-center text-success"><strong> ERROR:  ' +err +'</strong></p>');
    ...
    var addressFrom = $("#accountAddress").html();
    var firstNonce = web3.eth.getTransactionCount(addressFrom);
    var increaseCount = $("#newnameregister").val();
    alert(increaseCount);
    for(var i = 0; i < increaseCount; i++) {

    var addressNonce = web3.eth.getTransactionCount(addressFrom);
    addressNonce = addressNonce + i;
    alert(addressNonce);

    var nonce1 = padLeftEven(BNtoHex(new BigNumber(addressNonce)));
    var gasPrice1 = padLeftEven(BNtoHex(new BigNumber(0).plus(20000000000).toDigits(1))); 
    var gasLimit1 = padLeftEven(BNtoHex(new BigNumber(21000)));
    ...
    //alert("OK");
      ...
      web3.eth.sendRawTransaction(rdataReg.signed, function(err, result) {
        if(err) {
          textHTMLSTR += '<p class="text-center text-success"><strong> ERROR:  ';
          textHTMLSTR += err;
          textHTMLSTR += '</strong></p>';
 $("#regnamestatus").html(textHTMLSTR);
       }
        else {
          textHTMLSTR += '<p class="text-center text-success"><strong> Transaction submitted. TX ID:  </strong><a href="http://gastracker.io/tx/';
          textHTMLSTR += result;
          textHTMLSTR += '">';
          textHTMLSTR += result;
          textHTMLSTR += '</a></p>';
