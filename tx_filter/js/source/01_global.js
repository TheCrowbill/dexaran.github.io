...
function subPaneNavigate(showEleId, activeEleId) {
	$("#contract-by-ABI").hide();
	$("#contract-by-name").hide();
	$("#" + showEleId).show();
...
function bindElements() {
	...
	$("#interact-contract-by-abi").click(function() {
		$("#interact-contract-by-name").parent().removeClass('active');
	});

	$("#interact-contract-by-name").click(function() {
		$("#interact-contract-by-abi").parent().removeClass('active');
	});
	...
	$(".subPtabs").each(function(index) {
		$(this).click(function() {
			subPaneNavigate($(this).attr('showId'), this.id);
		});
	});
	$(".ptabs").each(function(index) {
		$(this).click(function() {
...
$('input[id=advancedTxOptionsContract]').change(function() {
	...
	$('input[type=radio][name=typeOfKeyRadio]').change(function() {
		PrivKey = "";
		currentAccount="";
		$("#mainHeader").html(`You have no currently decrypted accounts`);
		$('#fuploadStatus').empty();
...
function walletDecryptSuccess() {
	...
	currentAccount = formatAddress(strPrivateKeyToAddress(PrivKey), 'hex');
	$("#mainHeader").html(`<small> Your current account: </small>` + currentAccount);
	setWalletBalance();
